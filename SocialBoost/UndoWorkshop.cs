...
internal static class UndoWorkshop {


	public static async Task<string?> NaoSeguirOficinaPerfilSteam(Bot bot, EAccess access, string urlPerfil, string steamAlvo) {
		...
		bot.ArchiLogger.LogGenericInfo($"SocialBoost|WORKSHOP|UNFOLLOW => {steamAlvo} (Enviando)");

		if (string.IsNullOrEmpty(sessionId)) {
			return Commands.FormatBotResponse(Strings.BotLoggedOff, bot.BotName);
		}

		Uri request = new($"{urlPerfil}/unfollowuser");
		...
		bot.ArchiLogger.LogGenericInfo($"SocialBoost|WORKSHOP|UNFOLLOW => {steamAlvo} (OK)");
		return bot.Commands.FormatBotResponse(postSuccess ? $"{Strings.Success.Trim()} — ID: {steamAlvo} — Follower" : Strings.WarningFailed);

	}

	public static async Task<string?> RemoverFollowID64(EAccess access, ulong steamID, string botNames, string argument) {
		...
		bool? logger = await DSKLogger.CompartilharAtividade($"Workshop-UNFOLLOW-{argument2}").ConfigureAwait(false);
		IList<string?> results = await Utilities.InParallel(bots.Select(bot => NaoSeguirOficinaPerfilSteam(bot, Commands.GetProxyAccess(bot, access, steamID), argument, argument2))).ConfigureAwait(false);
